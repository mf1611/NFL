...
def static_features(df):
    ...
    static_df['Dir_cos'] = np.cos(static_df['Dir'])
    static_df['Dir_sin'] = np.sin(static_df['Dir'])

    static_df['S_cos'] = np.abs(static_df['S'] * np.cos(static_df['Dir']))
    static_df['S_sin'] = np.abs(static_df['S'] * np.sin(static_df['Dir']))

    static_df['A_cos'] = np.abs(static_df['A'] * np.cos(static_df['Dir']))
    static_df['A_sin'] = np.abs(static_df['A'] * np.sin(static_df['Dir']))
    ...
    static_df["diffScoreBeforePlay"] = train["HomeScoreBeforePlay"] - train["VisitorScoreBeforePlay"]
...
num += ['Age','PlayerHeight','Dir_cos','Dir_sin','S_cos','S_sin','diffScoreBeforePlay','DefendersInTheBox', 'Quarter', 'Down','Distance']
