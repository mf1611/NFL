...
#         return defense
    ...
    def add_features(df):
        ...
        for i, playid in enumerate(list(df_nn['PlayId'].unique())):
            ...
            df_nn.loc[df_nn['PlayId']==playid, 'X_minInVoronoi'] = vor.iloc[i].vertices[vor.iloc[i].regions[vor.iloc[i].point_region[0]]][:,1].max()  # Rusherのボロノイ領域の最大のy座標
            df_nn.loc[df_nn['PlayId']==playid, 'X_minInVoronoi'] = vor.iloc[i].vertices[vor.iloc[i].regions[vor.iloc[i].point_region[0]]][:,1].min()  # Rusherのボロノイ領域の最小のy座標
...
output_tmp = pd.DataFrame({'file': [os.path.basename(__file__)], 'CV_score': [np.mean(score)], 'LB': [0.01439]})
