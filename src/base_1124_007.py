...
if  TRAIN_OFFLINE==False:
    for k in range(2):
        ...
        for k_fold, (tr_inds, val_inds) in enumerate(kfold.split(X, yards)):
            ...
            tr_x,tr_y,tr_yards = X[tr_inds],y[tr_inds],yards[tr_inds]
            val_x,val_y,val_yards = X[val_inds],y[val_inds],yards[val_inds]

            #################################################################
            val_cnts = tr_yards.value_counts()
            ratio = {}
            for ya in tr_yards.unique():
                if val_cnts.values[val_cnts.index==ya][0] >= 100:
                    ratio[ya] = val_cnts.values[0]
                else:
                    ratio[ya] = val_cnts.values[val_cnts.index==ya][0]

            ros = RandomOverSampler(ratio=ratio, random_state = 71)
            tr_x, yards_resampled = ros.fit_sample(tr_x, tr_yards)

            tr_y = np.zeros((yards_resampled.shape[0], 199))
            for idx, target in enumerate(list(yards_resampled)):
