...
# Sの標準化を、2017年と2018年に分けた場合、大幅に悪化
...
def create_features(df, deploy=False):
    ...
    if not deploy:
        ...
        df.loc[df.Season<=2017, ['S']] = scaler_s_2017.fit_transform(df.loc[df.Season<=2017, ['S']])

        scaler_s = StandardScaler()
        df.loc[df.Season>2017, ['S']] = scaler_s.fit_transform(df.loc[df.Season>2017, ['S']])

    else:
        df.loc[df.Season<=2017, ['S']] = scaler_s_2017.transform(df.loc[df.Season<=2017, ['S']])
        df.loc[df.Season>2017, ['S']] = scaler_s.transform(df.loc[df.Season>2017, ['S']])
...
num_left = [col for col in num if col not in ['S']]
scaler = StandardScaler()
X[num_left] = scaler.fit_transform(X[num_left])
