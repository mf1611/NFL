...
usecols_x = [col for col in train_basetable.columns if '_x' in col]
usecols_y = [col for col in train_basetable.columns if '_y' in col]
assert len(usecols_x)==len(usecols_y)
for (c_x, c_y) in zip(usecols_x, usecols_y):
    train_basetable['%s_rate'%(c_x.replace('_x',''))] = 0
    train_basetable.loc[train_basetable[c_y]!=0, '%s_rate'%(c_x.replace('_x',''))] = train_basetable.loc[train_basetable[c_y]!=0, c_x] / train_basetable.loc[train_basetable[c_y]!=0, c_y]
