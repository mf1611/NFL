...
#         return defense
    ...
    def add_features(df):
        ...
        defense = df[df['Team'] != df['RusherTeam']].groupby(['GameId','PlayId'])\
                         .agg({
                         ...
                         .reset_index()
        defense.columns = ['GameId','PlayId','def_min_dist','def_max_dist','def_mean_dist','def_std_dist', 'def_min_X', 'def_max_X', 'def_mean_X', 'def_std_X', 'def_min_Y', 'def_max_Y', 'def_mean_Y', 'def_std_Y']
        ...
        offense = df[df['Team'] != df['RusherTeam']].groupby(['GameId','PlayId'])\
                         ...
                         .reset_index()
        offense.columns = ['GameId','PlayId','off_min_dist','off_max_dist','off_mean_dist','off_std_dist', 'off_min_X', 'off_max_X', 'off_mean_X', 'off_std_X', 'off_min_Y', 'off_max_Y', 'off_mean_Y', 'off_std_Y']

        df_summary = pd.merge(offense, defense, on=['GameId','PlayId'])

        # Positionごと
        map_defense = {'DE':'DL', 'DT':'DL','NG':'DL', 'NT':'DL', 'DL':'DL',
               'MLB':'LB','ILB':'LB','OLB':'LB','LB':'LB',
               'FS':'DB','SS':'DB', 'S':'DB', 'CB':'DB','SAF':'DB', 'DB':'DB',
               'FB':'Off','WR':'Off','G':'Off','C':'Off','OT':'Off'  # オフェンス！？
               }
        map_offense = {
            'G':'OL','C':'OL','OG':'OL', 'OT':'OL', 'T':'OL',
            'TE':'TE', # OLの端
            'WR':'WR', # サイド
        ...
        for pos in ['DL','LB','DB']:
            if df[df.Position==pos].shape[0]>0:
                pos_def = df[(df['Team'] != df['RusherTeam']) & (df['Position']==pos)]\
                    ...
                    .reset_index()
                pos_def.columns = ['GameId','PlayId','def_%s_min_dist'%pos,'def_%s_max_dist'%pos,'def_%s_mean_dist'%pos,'def_%s_std_dist'%pos,'def_%s_min_X'%pos, 'def_%s_max_X'%pos, 'def_%s_mean_X'%pos, 'def_%s_std_X'%pos, 'def_%s_min_Y'%pos, 'def_%s_max_Y'%pos, 'def_%s_mean_Y'%pos, 'def_%s_std_Y'%pos]
            ...
            if df[df.Position==pos].shape[0]>0:
                if pos!='WR':
                    pos_off = df[(df['Team'] == df['RusherTeam']) & (df['Position']==pos)]\
                        ...
                        .reset_index()
                    pos_off.columns = ['GameId','PlayId','off_%s_min_dist'%pos,'off_%s_max_dist'%pos,'off_%s_mean_dist'%pos,'off_%s_std_dist'%pos,'off_%s_min_X'%pos, 'off_%s_max_X'%pos, 'off_%s_mean_X'%pos, 'off_%s_std_X'%pos, 'off_%s_min_Y'%pos, 'off_%s_max_Y'%pos, 'off_%s_mean_Y'%pos, 'off_%s_std_Y'%pos]
                else:
                    pos_off = df[(df['Team'] == df['RusherTeam']) & (df['Position']==pos)]\
                        .groupby(['GameId','PlayId'])\
                        .agg({
                            'dist_from_rusher':['min','max'],
                            'X':['min','max'],
                            'Y':['min','max'],
                            })\
                        .reset_index()
                    pos_off.columns = ['GameId','PlayId','off_%s_min_dist'%pos,'off_%s_max_dist'%pos,'off_%s_min_X'%pos, 'off_%s_max_X'%pos, 'off_%s_min_Y'%pos, 'off_%s_max_Y'%pos]
...
cat = ['back_oriented_down_field', 'back_moving_down_field', 'OffenseFormation'] # ,'DefensePersonnel','OffensePersonnel']
...
num += ['def_min_dist', 'def_max_dist', 'def_mean_dist', 'def_std_dist']


# num_agg = ['min_dist', 'max_dist', 'mean_dist', 'std_dist']#, 'min_X', 'max_X', 'mean_X', 'std_X','min_Y', 'max_Y', 'mean_Y', 'std_Y']
# pos_def = ['DB','LB','DL']
# pos_off = ['OL','RB','TE']
...
#     for col in num_agg:
#         if od=='off':
#             num += ['%s_'%(od)+'%s_'%(pos)+col for pos in pos_off]
#         elif od=='def':
#             num += ['%s_'%(od)+'%s_'%(pos)+col for pos in pos_def]
#num += ['off_WR_%s'%col for col in ['min_dist', 'max_dist','min_X', 'max_X','min_Y', 'max_Y']]
...
cols_rm = ['Yards','GameId','PlayId','Team','Season','NflId','NflIdRusher','PlayerHeight','PlayerWeight']
features =[col for col in X.columns if col not in cols_rm]

cat_lgb = cat + list(X.select_dtypes('object').columns)
X[cat_lgb] = X[cat_lgb].astype('category')
...
lgbs = []
oof = np.zeros(len(X))
for i_, (tr_idx, val_idx) in enumerate(kf.split(X, yards, X['GameId'])):
    print(f'Fold : {i_+1}')
    tr_data = lgb.Dataset(X[features].iloc[tr_idx], label=yards[tr_idx])
    val_data = lgb.Dataset(X[features].iloc[val_idx], label=yards[val_idx])

    num_round = 10000
    model_lgb = lgb.train(params, tr_data, num_round, valid_sets = [tr_data, val_data], verbose_eval=100, early_stopping_rounds=100)
    oof[val_idx] = model_lgb.predict(X[features].iloc[val_idx], num_iteration=model_lgb.best_iteration)
    lgbs.append(model_lgb)
...
def model_NN():
    ...
    input_lgb = Input(shape=(1,))
    inputs.append(input_lgb)
...
for i_, (tdx, vdx) in enumerate(kf.split(X, y, X['GameId'])):
    ...
    X_train = [np.absolute(X_train[i]) for i in cat] + [X_train[num]] + [X_train['pred_lgb']]
    X_val = [np.absolute(X_val[i]) for i in cat] + [X_val[num]] + [X_val['pred_lgb']]
    model = model_NN()
...
#     for c in set(X.columns) - set(basetable.columns):
#         basetable[c] = 0

#     #############################
#     basetable[cat_lgb] = basetable[cat_lgb].astype('category')
#     basetable['pred_lgb'] = np.mean([model_lgb.predict(basetable[features]) for model_lgb in lgbs], axis=0)
#     basetable[cat] = basetable[cat].astype('int')
#     #############################
