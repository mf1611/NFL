...
def add_features(df):
    ...
    df_summary = df[df['Team'] != df['RusherTeam']].groupby(['GameId','PlayId'])\
                        .agg({
                            'dist_from_rusher':['min','max','mean','median','std'],
                            'X': ['mean','median'],
                            'Y': ['mean','median']
                        })\
                        .reset_index()
    ...
    offense = df[(df['Team'] == df['RusherTeam'])].groupby(['GameId','PlayId'])\
                        ...
                        .reset_index()
    offense.columns = ['GameId','PlayId','off_max_dist','off_mean_dist','off_med_dist','off_std_dist','off_X_mean','off_X_med', 'off_Y_mean','off_Y_med']

    offense_close = df[(df['Team'] == df['RusherTeam']) & (df['NflId']!=df['NflIdRusher'])].groupby(['GameId','PlayId'])\
                        .agg({
    ...
    # def_close = df[df['Team']!=df['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: d[d.dist_from_rusher==d.dist_from_rusher.min()][['X','Y','S','Dir','dist_from_rusher']].mean()).reset_index()
    # def_close.columns = ['GameId','PlayId','X_def','Y_def','S_def','Dir_def','dist_from_rusher_def']
    # off_close = df[(df['Team']==df['RusherTeam']) & (df['NflId']!=df['NflIdRusher'])].groupby(['GameId','PlayId']).apply(lambda d: d[d.dist_from_rusher==d.dist_from_rusher.min()][['X','Y','S','Dir','dist_from_rusher']].mean()).reset_index()
    # off_close.columns = ['GameId','PlayId','X_off','Y_off','S_off','Dir_off','dist_from_rusher_off']

    # df_summary = pd.merge(df_summary, def_close, on=['GameId','PlayId'])
    # df_summary = pd.merge(df_summary, off_close, on=['GameId','PlayId'])


    # df_summary['diff_off_def_Y'] = df_summary['Y_off'] - df_summary['Y_def']
    # df_summary['diff_off_def_Y_std'] = df_summary['diff_off_def_Y'] / np.sqrt(df_summary['dist_from_rusher_off']*df_summary['dist_from_rusher_def'])
    # df_summary['diff_Y_def_close_mean'] = df_summary['Y_def'] - df_summary['def_Y_mean']
    # df_summary['diff_Y_def_close_mean_std'] = df_summary['diff_Y_def_close_mean'] / df_summary['dist_from_rusher_def']
    # df_summary['diff_Y_off_close_mean'] = df_summary['Y_off'] - df_summary['off_Y_mean']
    # df_summary['diff_Y_off_close_mean_std'] = df_summary['diff_Y_off_close_mean'] / df_summary['dist_from_rusher_off']

    # df_summary['diff_off_def_X'] = df_summary['X_off'] - df_summary['X_def']
    # df_summary['diff_off_def_X_std'] = df_summary['diff_off_def_X'] / np.sqrt(df_summary['dist_from_rusher_off']*df_summary['dist_from_rusher_def'])
    # df_summary['diff_X_def_close_mean'] = df_summary['X_def'] - df_summary['def_X_mean']
    # df_summary['diff_X_def_close_mean_std'] = df_summary['diff_X_def_close_mean'] / df_summary['dist_from_rusher_def']
    # df_summary['diff_X_off_close_mean'] = df_summary['X_off'] - df_summary['off_X_mean']
    # df_summary['diff_X_off_close_mean_std'] = df_summary['diff_X_off_close_mean'] / df_summary['dist_from_rusher_off']




    # for c in [0,1]:
    #     def_agg = df[(df['Team'] != df['RusherTeam']) & (df['class']==c)].groupby(['GameId','PlayId']).agg({
    #         'dist_from_rusher': ['min','max','mean','std'],
    #         'Dir':['mean']
    #     }).reset_index()
    #     def_agg.columns = ['GameId','PlayId','def%d_min_dist'%c,'def%d_max_dist'%c,'def%d_mean_dist'%c,'def%d_std_dist'%c,'def%d_Dir_mean'%c]

    #     off_agg = df[(df['Team'] == df['RusherTeam']) & (df['class']==c)].groupby(['GameId','PlayId']).agg({
    #         'dist_from_rusher': ['min','max','mean','std'],
    #         'Dir':['mean']
    #     }).reset_index()
    #     off_agg.columns = ['GameId','PlayId','off%d_min_dist'%c,'off%d_max_dist'%c,'off%d_mean_dist'%c,'off%d_std_dist'%c,'off%d_Dir_mean'%c]

    #     df_summary = pd.merge(df_summary, def_agg, on=['GameId','PlayId'])
    #     df_summary = pd.merge(df_summary, off_agg, on=['GameId','PlayId'])
...
def create_features(df, deploy=False):
    ...
    # 最も近い人が、defかoffかのフラグ
    basetable['flag_closest_def'] = (basetable.min_dist==basetable.def_min_dist) * 1

    #basetable['YardLine_final'] = (basetable['YardLine'] > 70) * 1
...
num += ['off_max_dist','off_mean_dist','off_std_dist']
num += ['dist_between_def_off_std']

num += ['def_mean_rate','off_mean_rate']#,'def_mean_tot_std','off_mean_tot_std']

num += ['diff_dist_def_max_med_rate','diff_Y_def_max_med_rate']


# num += ['diff_off_def_X_std','diff_X_def_close_mean_std','diff_X_off_close_mean_std','diff_off_def_Y_std','diff_Y_def_close_mean_std','diff_Y_off_close_mean_std']
