...
def add_features(df):
    ...
    df_def_close_min = other_players[other_players['Team']!=other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:2,-1])).min()).reset_index()
    df_def_close_min.columns = ['GameId','PlayId','dist_def_close_min']
    df_def_close_mean = other_players[other_players['Team']!=other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:2,-1])).mean()).reset_index()
    df_def_close_mean.columns = ['GameId','PlayId','dist_def_close_mean']
