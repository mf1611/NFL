...
def add_features(df):
    ...
    df_def_close_min = other_players[other_players['Team']!=other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:5,-1])).min()).reset_index()
    df_def_close_min.columns = ['GameId','PlayId','def_min_dist']
    df_def_close_max = other_players[other_players['Team']!=other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:5,-1])).max()).reset_index()
    df_def_close_max.columns = ['GameId','PlayId','def_max_dist']
    df_def_close_mean = other_players[other_players['Team']!=other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:5,-1])).mean()).reset_index()
    df_def_close_mean.columns = ['GameId','PlayId','def_mean_dist']
    df_def_close_std = other_players[other_players['Team']!=other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:5,-1])).std()).reset_index()
    df_def_close_std.columns = ['GameId','PlayId','def_std_dist']

    df_off_close_min = other_players[other_players['Team']==other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:5,-1])).min()).reset_index()
    df_off_close_min.columns = ['GameId','PlayId','off_min_dist']
    df_off_close_max = other_players[other_players['Team']==other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:5,-1])).max()).reset_index()
    df_off_close_max.columns = ['GameId','PlayId','off_max_dist']
    df_off_close_mean = other_players[other_players['Team']==other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:5,-1])).mean()).reset_index()
    df_off_close_mean.columns = ['GameId','PlayId','off_mean_dist']
    df_off_close_std = other_players[other_players['Team']==other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:5,-1])).std()).reset_index()
    df_off_close_std.columns = ['GameId','PlayId','off_std_dist']


    # print(other_players.groupby(['GameId','PlayId']).mean().iloc[:3,5])
    # print(other_players.groupby(['GameId','PlayId']).mean().iloc[:3,3:])

    # df_def_close_Dir_mean = other_players.groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:3,5])[d['Team']!=d['RusherTeam']]).mean()).reset_index()
    # df_def_close_Dir_mean.columns = ['GameId','PlayId','Dir_def_close_mean']
    # df_off_close_Dir_mean = other_players.groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:3,5])[d['Team']==d['RusherTeam']]).mean()).reset_index()
    # df_off_close_Dir_mean.columns = ['GameId','PlayId','Dir_off_close_mean']
    ...
    # df_def_close_Dir_mean = other_players[other_players['Team']!=other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:2,5])).mean()).reset_index()
    # df_def_close_Dir_mean.columns = ['GameId','PlayId','Dir_def_close_mean']
    # df_off_close_Dir_mean = other_players[other_players['Team']==other_players['RusherTeam']].groupby(['GameId','PlayId']).apply(lambda d: ((d.iloc[:2,5])).mean()).reset_index()
    # df_off_close_Dir_mean.columns = ['GameId','PlayId','Dir_off_close_mean']

    # close_features = pd.merge(df_def_close_Dir_mean, df_off_close_Dir_mean, on=['GameId','PlayId'])

    close_features = pd.merge(df_def_close_min, df_def_close_max, on=['GameId','PlayId'])
    close_features = pd.merge(close_features, df_def_close_mean, on=['GameId','PlayId'])
    close_features = pd.merge(close_features, df_def_close_std, on=['GameId','PlayId'])
    close_features = pd.merge(close_features, df_off_close_max, on=['GameId','PlayId'])
    ...
    close_features = pd.merge(close_features, df_off_close_std, on=['GameId','PlayId'])
    # close_features = pd.merge(close_features, df_def_close_Dir_mean, on=['GameId','PlayId'])
    # close_features = pd.merge(close_features, df_off_close_Dir_mean, on=['GameId','PlayId'])
...
def create_features(df, deploy=False):
    ...
    # basetable['close_def_off_mean_rate'] = basetable['dist_def_close_mean'] / basetable['dist_off_close_mean']
    # basetable['close_def_off_min_rate'] = basetable['dist_def_close_min'] / basetable['dist_off_close_min']

    # basetable['Dir_def_off_similarity'] = np.cos(abs(basetable['Dir_def_close_mean']-(basetable['Dir_def_close_mean']-math.pi)*2 -basetable['Dir_off_close_mean']))
    # basetable['Dir_def_rusher_similarity'] = np.cos(abs(basetable['Dir_def_close_mean']-(basetable['Dir_def_close_mean']-math.pi)*2 -basetable['Dir']))
    # basetable[['Dir_def_off_similarity','Dir_def_rusher_similarity']].fillna(0, inplace=True)

    #print(basetable[['Dir_def_off_similarity','Dir_def_rusher_similarity']])
...
num += ['def_mean_rate','off_mean_rate']


# #num += ['dist_close_min','dist_close_mean','dist_close_std']
# num += ['dist_def_close_min','dist_off_close_min']
# num += ['close_def_mean_rate','close_off_mean_rate','close_def_off_mean_rate','close_def_off_min_rate']

#num += ['Dir_def_off_similarity','Dir_def_rusher_similarity']
