...
def defense_features(df):
    ...
    del defense, offense, offense_close
    gc.collect()
...
def create_features_3(df, rate, deploy=False):
    ...
    # Dir -> radianに
    df['Dir'] = np.mod(90 - df.Dir, 360) * math.pi/180.0
    df['X'] += rate*df['S'] * np.cos(df['Dir'])
    df['Y'] += rate*df['S'] * np.sin(df['Dir'])
    #df['S'] += 0.5*df['A']
    ##########################################################
    ##########################################################
...
def create_features_4(df, rate, deploy=False):
...
train_basetable_org = pd.merge(train_basetable, train_basetable_2, on=['GameId','PlayId'])

del train_basetable, train_basetable_2
gc.collect()
...
for rate1 in rate1_list:
    for rate2 in rate2_list:
        if (rate1<rate2) & ((rate1!=0.2) & (rate2!=0.4)):
            train_basetable = train_basetable_org.copy()
            train_basetable_3 = create_features_3(train, rate1, False)
            train_basetable_4 = create_features_4(train, rate2, False)
            ...
            if  TRAIN_OFFLINE==False:
                ...
                output_tmp = pd.DataFrame({'file': [os.path.basename(__file__)], 'CV_score': [np.mean(crps_csv)], 'LB': ['%f_%f'%(rate1,rate2)]})
