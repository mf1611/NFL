...
#             df_nn.loc[df_nn['PlayId']==playid, 'AreaInVoronoi'] = PolyArea(vor.iloc[i].vertices[vor.iloc[i].regions[vor.iloc[i].point_region[0]]])  # Rusherのボロノイ領域の面積
    ...
    def static_features(df):
        ...
        usecols = ['GameId','PlayId','X','Y','S','A','Dis','Orientation','Dir',
                    'YardLine','Quarter','Down','Distance','DefendersInTheBox','OffenseFormation',
                    'WindSpeed','WindDirection','GameWeather','StadiumType','Turf']
        static_features = df[df['NflId'] == df['NflIdRusher']][usecols].drop_duplicates()
...
cat = ['back_oriented_down_field', 'back_moving_down_field', 'OffenseFormation'] + ['WindDirection','GameWeather','StadiumType','Turf']
