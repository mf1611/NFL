...
def correction_df(df):
    ...
    df['X'] += (0.9*df['S']) * np.cos(df['Dir'])
    df['Y'] += (0.9*df['S']) * np.sin(df['Dir'])
...
def static_features(df):
    ...
    # add_new_features.append('PlayerHeight_dense')
    # ## Age
    # seconds_in_year = 60*60*24*365.25
    # df['PlayerAge'] = df.apply(lambda row: (row['TimeHandoff']-row['PlayerBirthDate']).total_seconds()/seconds_in_year, axis=1)
    # add_new_features.append('PlayerAge')
    ## WindSpeed
    add_new_features.append('WindSpeed')
    ## Orientation and Dir
    df["Dir_sin"] = np.sin(df["Dir"])
    df["Dir_cos"] = np.cos(df["Dir"])
    add_new_features.append("Dir_sin")
    add_new_features.append("Dir_cos")
    ## diff Score
    df["diffScoreBeforePlay"] = df["HomeScoreBeforePlay"] - df["VisitorScoreBeforePlay"]
    add_new_features.append("diffScoreBeforePlay")
...
num + ['DefendersInTheBox','WindSpeed_ob','Dir_sin','Dir_cos','diffScoreBeforePlay']
...
X[num+cat] = scaler.fit_transform(X[num+cat])
...
def model_NN():
    inp = Input(shape = (len(num+cat),))
    x = Dense(1024, input_dim=len(num+cat), activation='relu')(inp)
    x = Dropout(0.5)(x)
