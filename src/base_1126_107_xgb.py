...
param = {'objective':'multi:softprob',  # maximize=Trueの必要
         'num_class': 199,
...
models_xgb = []
oof_xgb = np.zeros((509762//22, 199))
y_xgb = yards+99
if  TRAIN_OFFLINE==False:
    for k in range(2):
        kfold = KFold(5, random_state = 42 + k, shuffle = True)
        for k_fold, (tr_idx, val_idx) in enumerate(kfold.split(X, y_xgb)):
            print(f'Fold : {k_fold+1}')
            tr_data = xgb.DMatrix(X[tr_idx], label=y_xgb[tr_idx])
            val_data = xgb.DMatrix(X[val_idx], label=y_xgb[val_idx])

            num_round = 10000
            model_xgb = xgb.train(param, tr_data, num_boost_round=num_round, evals=[(tr_data,'train'), (val_data,'val')], early_stopping_rounds=50, maximize=True)
            oof_lgb[val_idx,:] = model_xgb.predict(val_data, ntree_limit=model_xgb.best_ntree_limit).reshape(len(val_idx), 199)
    ...
    for k_fold, (tr_idx, val_idx) in enumerate(kfold.split(X, y_xgb)):
        ...
        num_round = 10000
        model_xgb = xgb.train(param, tr_data, num_boost_round=num_round, evals=[(tr_data,'train'), (val_data,'val')], early_stopping_rounds=50, maximize=True)
        oof_xgb[val_idx,:] = model_xgb.predict(val_data, ntree_limit=model_xgb.best_ntree_limit).reshape(len(val_idx), 199)
        models_xgb.append(model_xgb)


y_ans = np.zeros((509762//22, 199))
for i,p in enumerate(y_xgb-99):
    p+=99
    for j in range(199):
        if j>=p:
            y_ans[i][j]=1.0

y_oof = np.cumsum(oof_xgb, axis=1).clip(0,1)
print("XGB CVscore:",np.sum(np.power(y_oof-y_ans, 2))/(199*(509762//22)))
...
models_lgb = []
oof_lgb = np.zeros((509762//22, 199))
y_lgb = yards+99
if  TRAIN_OFFLINE==False:
    for k in range(2):
        kfold = KFold(5, random_state = 42 + k, shuffle = True)
        for k_fold, (tr_idx, val_idx) in enumerate(kfold.split(X, y_lgb)):
            ...
            oof_lgb[val_idx,:] = model_lgb.predict(X[val_idx], num_iteration=model_lgb.best_iteration)
...
y_oof = np.cumsum(oof_lgb, axis=1).clip(0,1)
...
#     lookahead.inject(model) # add into model
    ...
    y_valid = y_val
    y_true = np.clip(np.cumsum(y_valid, axis=1), 0, 1)
    y_predict = np.clip(np.cumsum(y_pred, axis=1), 0, 1)
    val_s = ((y_true - y_predict) ** 2).sum(axis=1).sum(axis=0) / (199 * x_val.shape[0])
...
print("mean_NN crps is %f"%np.mean(crps_csv))


#########################################################
# Ensemble割合

w_nn = [8,7,6,6,5,5,4,4]
w_lgb = [1,2,2,3,3,4,3,4]
w_xgb = [1,1,2,1,2,1,3,2]

y_oof = (oof_nn + oof_lgb + oof_xgb) /3
print("1:1:1: ",np.sum(np.power(y_oof-y_ans, 2))/(199*(509762//22)))

for (i,j,k) in zip(w_nn, w_lgb, w_xgb):
    y_oof = i* oof_nn + j* oof_lgb + k*oof_xgb
    print("%d:%d:%d: "%(i,j,k),np.sum(np.power(y_oof-y_ans, 2))/(199*(509762//22)))
...
def predict(x_te):
    ...
    # RF
    for l, model in enumerate(models_xgb):
        x_te = xgb.DMatrix(x_te)
        if l==0:
            y_pred_xgb = model.predict(x_te, ntree_limit=model.best_ntree_limit).reshape(len(x_te), 199) / len(models_xgb)
        else:
            y_pred_xgb += model.predict(x_te, ntree_limit=model.best_ntree_limit).reshape(len(x_te), 199) / len(models_xgb)

    y_pred = y_pred*0.6 + y_pred_lgb * 0.2 + + y_pred_xgb * 0.2
