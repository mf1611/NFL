...
def correction_df(df):
    ...
    cols = ['StadiumType','Turf','WindSpeed','WindDirection','GameWeather']
...
#             df_nn.loc[df_nn['PlayId']==playid, 'AreaInVoronoi'] = PolyArea(vor.iloc[i].vertices[vor.iloc[i].regions[vor.iloc[i].point_region[0]]])  # Rusherのボロノイ領域の面積
    ...
    def static_features(df):
        static_features = df[df['NflId'] == df['NflIdRusher']][[
            'GameId','PlayId','Team','X','Y','S','A','Dis','Orientation','Dir','OffenseFormation','YardLine','Quarter','Down','Distance','DefendersInTheBox','PlayerHeight','PlayerWeight','NflId','NflIdRusher','HomeScoreBeforePlay','VisitorScoreBeforePlay','DefensePersonnel','OffensePersonnel',
            'WindSpeed','WindDirection','GameWeather','StadiumType','Turf']].drop_duplicates()
...
'OffenseFormation',
'4th_above','4th_behind',
'GameWeather','StadiumType','Turf']
