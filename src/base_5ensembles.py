...
# num = num + ['def/off_min_dist','def/off_min_mean_dist','off/def_min_mean_dist','off_min_dist','off_max_dist','off_mean_dist','off_std_dist']
...
from sklearn.model_selection import GroupShuffleSplit
n_splits = 5
kf = GroupShuffleSplit(n_splits=n_splits, random_state=519)
for i_, (tdx, vdx) in enumerate(kf.split(X, y, X['GameId'])):
    print(f'Fold : {i_+1}')
    X_train, X_val, y_train, y_val = X.iloc[tdx], X.iloc[vdx], y[tdx], y[vdx]
    X_train = [np.absolute(X_train[i]) for i in cat] + [X_train[num]]
    ...
    model.fit(X_train, y_train, callbacks=[metric], epochs=100, batch_size=1024, verbose=False)
    score_ = crps(y_val, model.predict(X_val))
    model.save(f'keras_{(i_+10)}.h5')
    print(score_)
    score.append(score_)

from sklearn.model_selection import GroupShuffleSplit
n_splits = 5
kf = GroupShuffleSplit(n_splits=n_splits, random_state=429)
for i_, (tdx, vdx) in enumerate(kf.split(X, y, X['GameId'])):
    print(f'Fold : {i_+1}')
    X_train, X_val, y_train, y_val = X.iloc[tdx], X.iloc[vdx], y[tdx], y[vdx]
    X_train = [np.absolute(X_train[i]) for i in cat] + [X_train[num]]
...
from sklearn.model_selection import GroupShuffleSplit
n_splits = 5
kf = GroupShuffleSplit(n_splits=n_splits, random_state=129)
for i_, (tdx, vdx) in enumerate(kf.split(X, y, X['GameId'])):
    print(f'Fold : {i_+1}')
    X_train, X_val, y_train, y_val = X.iloc[tdx], X.iloc[vdx], y[tdx], y[vdx]
    X_train = [np.absolute(X_train[i]) for i in cat] + [X_train[num]]
