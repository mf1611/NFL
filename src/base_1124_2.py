...
def features_relative_to_back(df, carriers):
    ...
    player_distance = player_distance.groupby([
        ...
        .agg({'dist_to_back':['min','max','mean','median','std']}).reset_index()
    player_distance.columns = ['GameId','PlayId','back_from_scrimmage','back_oriented_down_field','back_moving_down_field','min_dist','max_dist','mean_dist','med_dist','std_dist']
...
def add_features(df):
    ...
    offense = df[df['Team'] == df['RusherTeam']].groupby(['GameId','PlayId'])\
                        ...
                        .reset_index()
    offense.columns = ['GameId','PlayId','off_min_dist','off_max_dist','off_mean_dist','off_med_dist','off_std_dist','off_X_mean','off_X_med', 'off_Y_mean','off_Y_med']
    ...
    df_summary['dist_med_between_def_off'] = df_summary[['def_X_med','def_Y_med','off_X_med','off_Y_med']].apply(lambda x: euclidean_distance(x[0],x[1],x[2],x[3]), axis=1)
    df_summary['dist_med_between_def_off_std'] = df_summary['dist_med_between_def_off'] / np.sqrt(df_summary['def_std_dist'] * df_summary['off_std_dist'])
...
def create_features(df, deploy=False):
    ...
    basetable['def_med_rate'] = basetable['def_med_dist'] / basetable['med_dist']  # 全体の平均の中での～
    basetable['off_med_rate'] = basetable['off_med_dist'] / basetable['med_dist']
    basetable['def_med_tot_std'] = basetable['def_med_dist'] / basetable['std_dist']  # 全体の平均の中での～
    basetable['off_med_tot_std'] = basetable['off_med_dist'] / basetable['std_dist']
...
num += ['def_med_dist','off_med_dist']
num += ['dist_med_between_def_off_std']
num += ['def_med_rate','off_med_rate']
