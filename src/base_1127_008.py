...
def defense_features(df):
    ...
    del defense,offense,offense_summary,offense_close
    gc.collect()
...
def create_features(df, deploy=False):
    ...
    del back_feats,rel_back,def_feats,rush_feats,static_feats
    gc.collect()
...
def create_features_5(df, deploy=False):
    ...
    # Dir -> radianに
    df['Dir'] = np.mod(90 - df.Dir, 360) * math.pi/180.0
    df['X'] += 5*df['S'] * np.cos(df['Dir'])
    df['Y'] += 5*df['S'] * np.sin(df['Dir'])
    #df['S'] += 0.5*df['A']
    ##########################################################
    ##########################################################
...
def create_features_6(df, deploy=False):
    ...
    basetable.columns = [col+'_6' if col not in ['GameId','PlayId'] else col for col in basetable.columns]
...
def create_features_7(df, deploy=False):
    ...
    # Dir -> radianに
    df['Dir'] = np.mod(90 - df.Dir, 360) * math.pi/180.0
    df['X'] += 15*df['S'] * np.cos(df['Dir'])
    df['Y'] += 15*df['S'] * np.sin(df['Dir'])
    #df['S'] += 0.5*df['A']
    ##########################################################
    ##########################################################
    ...
    basetable.columns = [col+'_7' if col not in ['GameId','PlayId'] else col for col in basetable.columns]
...
def create_features_8(df, deploy=False):
    ...
    # Dir -> radianに
    df['Dir'] = np.mod(90 - df.Dir, 360) * math.pi/180.0
    df['X'] += 20*df['S'] * np.cos(df['Dir'])
    df['Y'] += 20*df['S'] * np.sin(df['Dir'])
    #df['S'] += 0.5*df['A']
    ##########################################################
    ##########################################################
    ...
    basetable.columns = [col+'_8' if col not in ['GameId','PlayId'] else col for col in basetable.columns]
...
train_basetable_6 = create_features_6(train, False)
train_basetable_7 = create_features_7(train, False)
train_basetable_8 = create_features_8(train, False)
...
train_basetable = pd.merge(train_basetable, train_basetable_6, on=['GameId','PlayId'])
train_basetable = pd.merge(train_basetable, train_basetable_7, on=['GameId','PlayId'])
train_basetable = pd.merge(train_basetable, train_basetable_8, on=['GameId','PlayId'])
