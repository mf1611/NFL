...
cols_1 = [col for col in train_basetable.columns if col not in ['GameId','PlayId','Yards']]
cols_2 = [col for col in train_basetable_2.columns if col not in ['GameId','PlayId']]
cols_3 = [col for col in train_basetable_3.columns if col not in ['GameId','PlayId']]
cols_4 = [col for col in train_basetable_4.columns if col not in ['GameId','PlayId']]
...
# 3つの特徴量がある
# それらには、時系列がある
# なので、段階的に混ぜ合わせていくアプローチを考える

def get_model(x_tr,y_tr,x_val,y_val):

    inputs = []
    inp_1 = Input(shape = (len(cols_1),))
    inputs.append(inp_1)
    x = Dense(1024, input_dim=len(cols_1), activation='relu')(inp_1)
    x = Dropout(0.5)(x)
    ...
    inp_2 = Input(shape = (len(cols_2),))
    inputs.append(inp_2)
    x = Dense(512, input_dim=len(cols_2), activation='relu')(inp_2)
    x = Dropout(0.5)(x)
    x = BatchNormalization()(x)
    x = Dense(256, activation='relu')(x)
    x = Dropout(0.5)(x)
    ...
    inp_3 = Input(shape = (len(cols_3),))
    inputs.append(inp_3)
    x = Dense(512, input_dim=len(cols_3), activation='relu')(inp_3)
    x = Dropout(0.5)(x)
    x = BatchNormalization()(x)
    x = Dense(256, activation='relu')(x)
    x = Dropout(0.5)(x)
    ...
    inp_4 = Input(shape = (len(cols_4),))
    inputs.append(inp_4)
    x = Dense(512, input_dim=len(cols_4), activation='relu')(inp_4)
    x = Dropout(0.5)(x)
    x = BatchNormalization()(x)
    x = Dense(256, activation='relu')(x)
    x = Dropout(0.5)(x)
    x_4 = BatchNormalization()(x)
...
#     lookahead.inject(model) # add into model
    ...
    y_true = np.clip(np.cumsum(y_valid, axis=1), 0, 1)
    y_pred = np.clip(np.cumsum(y_pred, axis=1), 0, 1)
    val_s = ((y_true - y_pred) ** 2).sum(axis=1).sum(axis=0) / (199 * x_val[0].shape[0])
...
if  TRAIN_OFFLINE==False:
    for k in range(2):
        ...
        for k_fold, (tr_inds, val_inds) in enumerate(kfold.split(X, yards)):
            ...
            X_train, tr_y = X.iloc[tr_inds],y[tr_inds]
            X_val, val_y = X.iloc[val_inds],y[val_inds]

            tr_x = [X_train[cols_1],X_train[cols_2],X_train[cols_3],X_train[cols_4]]
            val_x = [X_val[cols_1],X_val[cols_2],X_val[cols_3],X_val[cols_4]]
