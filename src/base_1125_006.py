...
def create_features(df, deploy=False):
    ...
    def defense_features(df):
        ...
        defense = df[df['Team'] != df['RusherTeam']][['GameId','PlayId','X','Y','S','Dir','RusherX','RusherY','dist_from_rusher']]
        ...
        df_summary = defense.groupby(['GameId','PlayId'])\
                         .agg({'dist_from_rusher':['min','max','mean','median','std'],
                              ...
                              'Dir':['median']})\
                         .reset_index()
        df_summary.columns = ['GameId','PlayId','def_min_dist','def_max_dist','def_mean_dist','def_med_dist','def_std_dist','def_X_mean','def_X_med', 'def_Y_mean','def_Y_med','def_S_med','def_Dir_med']


        #####
        offense = df[df['Team'] == df['RusherTeam']][['GameId','PlayId','NflId','NflIdRusher','X','Y','S','Dir','RusherX','RusherY','dist_from_rusher']]
        offense_summary = offense.groupby(['GameId','PlayId'])\
                            ...
                            .reset_index()
        offense_summary.columns = ['GameId','PlayId','off_max_dist','off_mean_dist','off_med_dist','off_std_dist','off_X_mean','off_X_med', 'off_Y_mean','off_Y_med','off_Y_min','off_Y_max','off_S_med','off_Dir_med']
        ...
        offense_Ymax = df[(df['Team'] == df['RusherTeam'])].groupby(['GameId','PlayId'])['Y'].max().reset_index()
        offense_Ymax = pd.merge(df[(df['Team']==df['RusherTeam'])], offense_Ymax, on=['GameId','PlayId','Y'], how='inner').groupby(['GameId','PlayId']).agg({
                        'X':['mean'],
                        'Y':['mean'],
                        'S':['mean'],
                        'A':['mean'],
        ...
        offense_Ymax.columns = ['GameId','PlayId','X_Ymax','Y_Ymax','S_Ymax','A_Ymax','Dis_Ymax','Orientation_Ymax','Dir_Ymax']

        offense_Ymin = df[(df['Team'] == df['RusherTeam'])].groupby(['GameId','PlayId'])['Y'].min().reset_index()
        offense_Ymin = pd.merge(df[(df['Team']==df['RusherTeam'])], offense_Ymin, on=['GameId','PlayId','Y'], how='inner').groupby(['GameId','PlayId']).agg({
                        'X':['mean'],
                        'Y':['mean'],
                        'S':['mean'],
                        'A':['mean'],
        ...
        offense_Ymin.columns = ['GameId','PlayId','X_Ymin','Y_Ymin','S_Ymin','A_Ymin','Dis_Ymin','Orientation_Ymin','Dir_Ymin']
        ...
        # offense_Xmax = df[(df['Team'] == df['RusherTeam'])].groupby(['GameId','PlayId'])['X'].max().reset_index()
        # offense_Xmax = pd.merge(df[(df['Team']==df['RusherTeam'])], offense_Xmax, on=['GameId','PlayId','X'], how='inner').groupby(['GameId','PlayId']).agg({
        #                 'X':['mean'],
        #                 'Y':['mean'],
        #                 'S':['mean'],
        #                 'A':['mean'],
        ...
        #                 'Dir':['mean'],
        # }).reset_index()
        # offense_Xmax.columns = ['GameId','PlayId','X_Xmax','Y_Xmax','S_Xmax','A_Xmax','Dis_Xmax','Orientation_Xmax','Dir_Xmax']

        defense_closest = defense.groupby(['GameId','PlayId'])['dist_from_rusher'].min().reset_index()
        defense_closest = pd.merge(df[(df['Team']!=df['RusherTeam'])], defense_closest, on=['GameId','PlayId','dist_from_rusher'], how='inner').groupby(['GameId','PlayId']).agg({
                        'X':['mean'],
                        'Y':['mean'],
                        'S':['mean'],
                        'A':['mean'],
        ...
        defense_closest.columns = ['GameId','PlayId','X_def_closest','Y_def_closest','S_def_closest','A_def_closest','Dir_def_closest']

        offense_closest = offense[(offense['NflId']!=offense['NflIdRusher'])].groupby(['GameId','PlayId'])['dist_from_rusher'].min().reset_index()
        offense_closest = pd.merge(df[(df['Team']==df['RusherTeam'])], offense_closest, on=['GameId','PlayId','dist_from_rusher'], how='inner').groupby(['GameId','PlayId']).agg({
                        'X':['mean'],
                        'Y':['mean'],
                        'S':['mean'],
                        'A':['mean'],
        ...
        offense_closest.columns = ['GameId','PlayId','X_off_closest','Y_off_closest','S_off_closest','A_off_closest','Dir_off_closest']


        # 2番目に近い選手の属性
        defense = defense.sort_values(by=['GameId','PlayId','dist_from_rusher'])
        defense_closest2 = defense.groupby(['GameId','PlayId'])['dist_from_rusher'].apply(lambda d: d.iloc[1:].min()).reset_index()
        defense_closest2 = pd.merge(df[(df['Team']!=df['RusherTeam'])], defense_closest2, on=['GameId','PlayId','dist_from_rusher'], how='inner').groupby(['GameId','PlayId']).agg({
                        'X':['mean'],
                        'Y':['mean'],
                        'S':['mean'],
                        'A':['mean'],
        ...
        defense_closest2.columns = ['GameId','PlayId','X_def_closest2','Y_def_closest2','S_def_closest2','A_def_closest2','Dir_def_closest2']

        offense = offense.sort_values(by=['GameId','PlayId','dist_from_rusher'])
        offense_closest2 = offense[(offense['NflId']!=offense['NflIdRusher'])].groupby(['GameId','PlayId'])['dist_from_rusher'].apply(lambda d: d.iloc[1:].min()).reset_index()
        offense_closest2 = pd.merge(df[(df['Team']==df['RusherTeam'])], offense_closest2, on=['GameId','PlayId','dist_from_rusher'], how='inner').groupby(['GameId','PlayId']).agg({
                        'X':['mean'],
                        'Y':['mean'],
                        'S':['mean'],
                        'A':['mean'],
        ...
        offense_closest2.columns = ['GameId','PlayId','X_off_closest2','Y_off_closest2','S_off_closest2','A_off_closest2','Dir_off_closest2']


        df_summary = pd.merge(df_summary, offense_Ymax, on=['GameId','PlayId'])
        df_summary = pd.merge(df_summary, offense_Ymin, on=['GameId','PlayId'])
        #df_summary = pd.merge(df_summary, offense_Xmax, on=['GameId','PlayId'])
        df_summary = pd.merge(df_summary, defense_closest, on=['GameId','PlayId'])
        df_summary = pd.merge(df_summary, offense_closest, on=['GameId','PlayId'])
        df_summary = pd.merge(df_summary, defense_closest2, on=['GameId','PlayId'])
        df_summary = pd.merge(df_summary, offense_closest2, on=['GameId','PlayId'])


        df_summary.drop(['def_med_dist','off_med_dist','def_X_mean','def_X_med','def_Y_mean','def_Y_med','off_X_mean','off_X_med', 'off_Y_mean','off_Y_med','off_Y_min','off_Y_max'], axis=1, inplace=True)
...
def create_features_2(df, deploy=False):
    ...
    def defense_features(df):
        ...
        offense_summary = offense.groupby(['GameId','PlayId'])\
                            ...
                            .reset_index()
        offense_summary.columns = ['GameId','PlayId','off_max_dist','off_mean_dist','off_med_dist','off_std_dist','off_X_mean','off_X_med', 'off_Y_mean','off_Y_med','off_S_med','off_Dir_med']
